...
import { DISTANCE_UNIT_ALL, DISTANCE_UNIT_TO_LONG_STR, DISTANCE_UNIT_TO_SHORT_STR, DistanceUnitConversionState, DistanceUnits } from "@/services/distance-unit-conversion";
import { ReactNode, useState } from "react";

export function PageClient() {
    const [value, updateValue] = useState<string>("")
    const [selectedUnit, updateSelectedUnit] = useState<DistanceUnits>(DistanceUnits.ItnInch)
        ...
        buttons.push(
            (
                <UnitButton
                    ...
                    onClick={() => { updateSelectedUnit(unit) }} />
            )
        )
    });
...
function UnitButton(
    ...
    if (props.selected) {
        return (
            <button className='border-2 border-blue-500 rounded-md' onClick={props.onClick}>
                {content}
            </button>
        )
    } else {
        return (
            <button className='border-2 border-blue-200 rounded-md' onClick={props.onClick}>
                {content}
            </button>
        )
    }
