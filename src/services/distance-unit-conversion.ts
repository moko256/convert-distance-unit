...
export const DISTANCE_UNIT_ALL: Array<DistanceUnits> = Array.from(
  CONVERSION_TABLE_TO_METER.keys(),
);
...
export class DistanceUnitConversionState {
  ...
  private convert(): Map<DistanceUnits, number> {
    const fromValueInMeter =
      this.fromValue * (CONVERSION_TABLE_TO_METER.get(this.fromUnit) ?? 0);

    const map = new Map<DistanceUnits, number>();

    CONVERSION_TABLE_TO_METER.forEach(
      (value: number, key: DistanceUnits, _map: any) => {
